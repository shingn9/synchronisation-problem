...
pthread_mutex_t repeat_IDs_mutex;
...
void *teacher_routine(void *arg) {
    ...
    for (int i = 0; i < no_of_students; ++i) {
        ...
        pthread_cond_signal(&waiting_repeating_ids);  // Signal the corresponding student
...
void *student_routine(void *arg) {
    ...
    pthread_mutex_lock(&students_mutex);
    printf("Student [%d]: I have arrived and wait to be assigned to a group.\n", student->id);
    students_arrived++;
